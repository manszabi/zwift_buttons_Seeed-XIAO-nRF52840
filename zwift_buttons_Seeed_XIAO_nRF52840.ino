...
static bool hidPress(bool consumer, uint8_t modifier, uint16_t code, uint8_t mask) {
  // A média (consumer) jelentés nem visz módosítót: a consumerKeyPress() csak a
  // usage kódot küldi ki. Ezért a módosítót MÁR ITT nullázzuk, egyetlen helyen —
  // különben a lenyomat (heldModifier) 0 lenne, a hidSameAsHeld() viszont a
  // művelet nem nulla módosítójával hasonlítana, sosem egyezne, és a nyomva
  // tartott ismétlés minden körben fölöslegesen felengedne és újra lenyomna.
  if (consumer) modifier = 0;
  ...
  hasConsumerKeyPressed = consumer;
  heldCode = code;
  heldModifier = modifier;   // consumer esetén már 0 (lásd a függvény elejét)
  releaseRetries = 0;
  return true;
}
